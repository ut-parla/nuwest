...
if $use_gpu; then
    ...
    docker run --runtime=nvidia --gpus all --volume $(pwd):/app --workdir /app utaustin/nuwest nsys profile --force-overwrite true --trace nvtx,cuda,python-gil -o reports/${name}.nsys-rep python $@
else
    echo "Running without GPU support..."
    docker run --volume $(pwd):/app --workdir /app utaustin/nuwest nsys profile --force-overwrite true --trace nvtx,cuda,python-gil -o reports/${name}.nsys-rep python $@
fi
