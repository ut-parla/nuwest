...
@pk.workunit(
    ...
    d_collct_ar=pk.ViewTypeInfo(layout=pk.LayoutRight, space=pk.CudaSpace),
    ...
    d_currxbins=pk.ViewTypeInfo(space=pk.CudaSpace),
    d_forE_xbins=pk.ViewTypeInfo(space=pk.CudaSpace),
    d_curr_count=pk.ViewTypeInfo(space=pk.CudaSpace),
    d_g0_ar=pk.ViewTypeInfo(space=pk.CudaSpace),
    d_g1_ar=pk.ViewTypeInfo(space=pk.CudaSpace),
    d_g2_ar=pk.ViewTypeInfo(space=pk.CudaSpace),
    d_g3_ar=pk.ViewTypeInfo(space=pk.CudaSpace),
)
...
def electron_kernel_1D(
    ...
    d_collct_ar: pk.View2D[float], 
    d_E_ar: pk.View1D[float], 
    d_ns_ar: pk.View1D[float], 
    d_R_vec: pk.View2D[float], 
    d_currxbins: pk.View1D[pk.int64],
    d_forE_xbins: pk.View1D[pk.int64], 
    nn: float, 
    dt: float, 
    d_curr_count: pk.View1D[pk.int64],
    d_g0_ar: pk.View1D[float], 
    d_g1_ar: pk.View1D[float], 
    d_g2_ar: pk.View1D[float], 
    d_g3_ar: pk.View1D[float],
    stride: int,
):
    ...
    for mystep in range(num_steps):
        ...
        for i in range(tid, Nc, stride):
            ...
            #x0 = 0.2347
            #x1 = 11.71
            #sig_g0 = 9.9e-20*(a1+b1*(math.log(temp_ev/x1))**2)/(1+b1*(math.log(temp_ev/x1))**2)*(a0+b0*(math.log(temp_ev/x0))**2)/(1+b0*(math.log(temp_ev/x0))**2)/(1+c*temp_ev**d)
...
#            i10 = 59.158
#
#            log_sig_g1 = (e_exc < e_el < i1) * (((a1*log_e_el + b1)* log_e_el + c1)) + \
...
#                sig_g1 = 0.
            ...
            #if (e_el > 15.76):
            #    sig_g2 = 10*(2.86e-20 / math.log(90-15.76)) * math.log((e_el-15.76 + 1)) * math.exp(-1e-2*((e_el-90)/90)**2)
            #else:
            #    sig_g2 = 0.
...
#            i6 = 6.510018
#            i7 = 11.95935
#            log_sig_g3 = (e_ion_step < e_el < i1) * (((a1*log_e_el + b1)* log_e_el + c1)) + \
#                         (i1 <= e_el < i2) *        (((a2*log_e_el + b2)* log_e_el + c2)) + \
#                         (i2 <= e_el < i3) *        (((a3*log_e_el + b3)* log_e_el + c3)) + \
#                         (i3 <= e_el < i4) *        (((a4*log_e_el + b4)* log_e_el + c4)) + \
#                         (i4 <= e_el < i5) *        (((a5*log_e_el + b5)* log_e_el + c5)) + \
...
#                sig_g3 = 0.
            ...
            if (d_R_vec[i][0] < sig_range_g0):
                ...
                #cos_chi = 1 - (2*d_R_vec[i][1]) / (1 + 8*e_el*(1 - d_R_vec[i][1]))
            ...
            ## G1: EXCITATION
                ...
                #print("DOING G1 from energy ", e_el) 
                ## Original electron deflection direction (vscat)      
                #cos_chi = 1 - (2*d_R_vec[i][1]) / (1 + 8*e_el*(1 - d_R_vec[i][1]))
                cos_chi: float = 2*d_R_vec[i][1] - 1
                chi: float = math.acos(cos_chi)
...
def electron_kernel(
    ...
    #mystream = cp.cuda.get_current_stream()
    #instance = pk.ExecutionSpaceInstance(pk.Cuda, mystream)

    pk.parallel_for(
        #pk.RangePolicy(instance,0,num_threads),
        num_threads,
        electron_kernel_1D,
        Nc=Nc,
